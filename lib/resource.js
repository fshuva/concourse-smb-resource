...
async function docheck(source) {
  ...
  const directoryArray = directory.trim().split('\n').map(line => {
    const [name, type, _, day, month, date, mtime, year] = line.trim().split(/\s+/);
    const dateStr = `${day} ${month} ${date} ${mtime} ${year}`;
    return {
      type,
      mtime:new Date(Date.parse(dateStr)).getTime(), 
      name
    };
  });
  ...
  const filteredFolders = directoryArray.filter(({ name }) => name.includes('build')).sort((a, b) => b.mtime - a.mtime);
  const latestFolder = filteredFolders.length > 0 ? filteredFolders[0].name : '';
  console.log(latestFolder);
  // return [{ ref: calcRef(JSON.stringify(filteredFolders)) }];

  // rETURN LATEST FOLDER
  return [{ "ref": calcRef(latestFolder)}];
}
...
async function doin(source, {files = [], skip = false} = {}, {ref = null} = {}, dest) {
  ...
  const { latestFolder} = await docheck(source);
  const latestFolderDest = path.join(dest, latestFolder);

  return {
    version: {
      ref: calcRef(latestFolder),
    },
    metadata: [
      { name: 'timestamp', value: Math.floor(Date.now() / 1000) },
      { name: 'latest-folder', value: latestFolder },
    ],
    dest: latestFolderDest,
  };
